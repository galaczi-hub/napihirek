...
Pontosan 5 hirt adj minden kategoriban (nem tobbet!). CSAK JSON, semmi mas.
"""
...
def get_news_from_groq(date_str):
    ...
    try:
        ...
        # Probaljuk megjavitani a csonkult JSON-t
        for ending in ['"}]}]}', '"}]}', '"}}', '"}']: 
            for cut in range(len(clean), max(len(clean)-500, 0), -1):
                try:
                    fixed = clean[:cut] + ending
                    result = json.loads(fixed)
                    print(f"JSON javitva: {cut} karakter + '{ending}'")
                    return result
                except Exception:
                    continue
        raise
